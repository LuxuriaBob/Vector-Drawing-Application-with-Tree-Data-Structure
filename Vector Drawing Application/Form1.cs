...
namespace Vector_Drawing_Application
{
    ...
    Math.Abs(startlocation.X - endlocation.X), Math.Abs(startlocation.Y - endlocation.Y), SelectedRect); //width and height is difference of coordinates
        ...
        string path = @"C:\Users\yalin\Desktop\NETCAD\deneme4\deneme4\deneme4\bin\Debug\output1.xml";
        /*
        private void WriteXML()
        {
            FileStream writer = File.Create(path1);
            serialiser.Serialize(writer, Rects);
            writer.Close();
        }

        private void ReadXML()
        {
            TextReader writer = new StreamReader(path1);
            List<GraphRect> deserializedList = (List<GraphRect>)serialiser.Deserialize(writer);
...
{
    public GraphRect Rect;
    public PointF StartRectPoint;
    public float WidthRect;
    public float HeightRect;
...
public class GraphRect
{
    ...
    public GraphRect getParent()
    {
        return Parent;
    }

    public void setParent(GraphRect Rect, GraphRect ParentRect)
    {
    ...
    public List<GraphRect> getChilds(GraphRect Rect)
    {
        return Rect.Childs;
    }

    public void setChild(GraphRect ParentRect, GraphRect Rect)
    {
        ParentRect.Childs.Add(Rect);
    }
}